...
inline KEY_CODE ToKeyCode(WPARAM wParam)
{
...
void Window::SetCallbacks(WindowCallbacks const& callbacks)
{
    m_Callbacks = callbacks;
}


bool Window::IsLoop() const
{
...
void Window::Sleep(u32 milliSeconds)
{
...
Size2D const& Window::GetClientSize() const
{
    return m_ClientSize;
}
...
{
    return reinterpret_cast<WindowHandle>(m_hWnd);
}


void Window::ShowMessageBox(std::string const& message, std::string const& caption)
{
...
{
    Window* pInstance = reinterpret_cast<Window*>(GetWindowLongPtrA(hWnd, GWLP_USERDATA));
...
LRESULT CALLBACK Window::WindowProcedureInstance(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
